...
export function SuggestionPanel({ suggestions }: SuggestionPanelProps) {
  ...
  const filteredSuggestions = suggestions.filter((s) => s.impressions >= minImpressions)
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6 h-full flex flex-col">
      <div className="mb-4">
        ...
        <div>
          <label className="text-xs text-muted-foreground block mb-2">Min Impressions: {minImpressions}</label>
          <input
            type="range"
            min="0"
            max="500000"
          ...
          filteredSuggestions.map((suggestion) => (
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <span
                  className={`inline-block px-2 py-0.5 rounded text-xs font-semibold capitalize ${typeColors[suggestion.type]}`}
                >
                  {suggestion.type}
                </span>
              </div>
              ...
              <p className="text-xs text-muted-foreground">Impressions: {suggestion.impressions.toLocaleString()}</p>
            </div>
          ))
        ) : (
          <p className="text-sm text-muted-foreground text-center py-4">No suggestions match your filters</p>
        )}
      </div>
    </div>
  )
